...
下面我们做几个**短**训练，每个用不同的 β（含 0 = 完全不拴），看 RM reward 和 KL 如何变化。

> 🤔 **先猜再跑**：四档 β = 0 / 0.02 / 0.1 / 0.5。预测两件事：
> 1. 哪一档的 RM reward 涨得**最高**？（提示：没有缰绳的马跑得最快——但它跑去哪了？）
> 2. β=0.5 这档的 reward 大概什么水平——高还是低？为什么"拴得太紧"也不行？
>
> <details><summary>写下预测再点开</summary>
>
> 期望的剧本：β=0 reward 最高但 response 开始退化（Ch11 的过优化曲线现场版）；β=0.02 平衡点（这也是 InstructGPT 的量级）；β=0.5 拴得太死——policy 被钉在 reference 附近，RM 分数涨不动。**β 的本质是"你信 RM 几分"**：信得多就少拴，信得少就多拴。
> </details>
