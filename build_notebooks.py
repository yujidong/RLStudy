...
第 1 章的 bandit 没有状态：你选 $a$，得到 $r$，仅此而已。
...
1. **状态怎么转移？** 选了动作 $a$ 之后，下一个状态是什么？
2. **奖励怎么和长期挂钩？** 当前一步的奖励和未来奖励如何权衡？
...
> Ch02 我们用矩阵法精确求解了 $V^\\pi$。这一章我们用**迭代算法**做同样的事——
> 这些算法的迭代形式正是后面 TD、DQN、PPO 的基础。
...
> Ch03 假设我们**知道** $P$ 和 $R$。这一章放弃这个假设——agent 必须**从与环境的真实交互样本中学习**。
> TD（Temporal Difference，时序差分）是 RL 中最核心的思想之一。
...
**这是 RL 教材最经典的图之一**。Sutton-Barto 19 状态随机游走：
